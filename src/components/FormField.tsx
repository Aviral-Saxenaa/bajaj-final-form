...
const FormField: React.FC<FormFieldProps> = ({ field, value, onChange, error }) => {
  ...
  const baseInputClasses =
    'w-full px-4 py-2 text-base border rounded-xl font-dm-sans transition focus:outline-none focus:ring-2 focus:ring-primary-400';
        ...
        return (
          <textarea
            className={`${baseInputClasses} min-h-[100px] ${
              error || localError ? 'border-red-500 ring-red-300' : 'border-gray-300'
            }`}
          ...
          <div className="space-y-2">
            {field.options?.map((option) => (
              <label key={option.value} className="flex items-center space-x-2">
                ...
                />
                <span className="font-dm-sans">{option.label}</span>
              </label>
            ))}
          </div>
        );
  ...
  return (
    <div className="mb-6">
      <label className="block mb-2 text-sm font-medium text-gray-700 font-dm-sans">
        {field.label}
        {field.required && <span className="text-red-500 ml-1">*</span>}
      </label>
      {renderField()}
      {(error || localError) && (
        <p className="mt-2 text-sm text-red-600 font-dm-sans">{localError || error}</p>
      )}
    </div>
  );
};
